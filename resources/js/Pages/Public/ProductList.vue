...
const hasAnySocial = computed(() => {
    try {
        return Boolean(props.store?.facebook_url || props.store?.instagram_url || props.store?.tiktok_url);
    } catch (e) {
        return false;
    }
});
...
const socialLinks = computed(() => {
    ...
    if (tt) links.push({ key: 'tt', href: tt, iconClass: 'text-white' });
    return links;
});
...
<template>
    ...
    <header class="bg-white shadow-sm sticky top-0 z-50">
        <nav class="container mx-auto px-6 py-4 flex items-center justify-between gap-2">
            ...
            <div class="flex items-center space-x-3 shrink-0">
                 <a v-if="store.facebook_url" :href="store.facebook_url" target="_blank" class="hidden md:inline-block text-gray-500 hover:text-blue-800">
                ...
                <a v-if="store.instagram_url" :href="store.instagram_url" target="_blank" class="hidden md:inline-block text-gray-500 hover:text-pink-600">
                ...
                <a v-if="store.tiktok_url" :href="store.tiktok_url" target="_blank" class="hidden md:inline-block text-gray-500 hover:text-black">
                ...
                <div class="border-l h-6 border-gray-300 hidden md:block"></div>
    ...
    <!-- FAB Social en móvil -->
    <div v-if="hasAnySocial" class="fixed bottom-6 right-6 z-50 md:hidden">
        <div class="relative">
            <!-- Burbujas -->
            <transition name="fade">
                <div v-if="showSocialFab" class="absolute right-0 bottom-0 flex flex-col items-end gap-3">
                    <a v-for="item in socialLinks" :key="item.key" :href="item.href" target="_blank" class="w-11 h-11 rounded-full bg-white/70 backdrop-blur ring-1 ring-white/60 flex items-center justify-center shadow-2xl active:scale-95">
                        ...
                        <svg v-else class="w-5 h-5 text-black" viewBox="0 0 24 24" fill="currentColor"><path d="M12.525.02c1.31-.02 2.61-.01 3.91.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.01-1.58-.31-3.15-.82-4.7-.52-1.56-1.23-3.04-2.1-4.42a.1.1 0 00-.2-.04c-.02.13-.03.26-.05.39v7.24a.26.26 0 00.27.27c.82.04 1.63.16 2.42.37.04.83.16 1.66.36 2.47.19.82.49 1.6.86 2.33.36.73.81 1.41 1.32 2.02-.17.1-.34.19-.51.28a4.26 4.26 0 01-1.93.52c-1.37.04-2.73-.06-4.1-.23a9.8 9.8 0 01-3.49-1.26c-.96-.54-1.8-1.23-2.52-2.03-.72-.8-1.3-1.7-1.77-2.69-.47-.99-.8-2.06-1.02-3.13a.15.15 0 01.04-.15.24.24 0 01.2-.09c.64-.02 1.28-.04 1.92-.05.1 0 .19-.01.28-.01.07.01.13.02.2.04.19.04.38.09.57.14a5.2 5.2 0 005.02-5.22v-.02a.23.23 0 00-.23-.23.2.2 0 00-.2-.02c-.83-.06-1.66-.13-2.49-.22-.05-.01-.1-.01-.15-.02-1.12-.13-2.25-.26-3.37-.44a.2.2 0 01-.16-.24.22.22 0 01.23-.18c.41-.06.82-.12 1.23-.18C9.9.01 11.21 0 12.525.02z"/></svg>
                    </a>
                </div>
            </transition>
